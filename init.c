...
{
    int id = philo->id - 1; // Adjusting for zero-based indexing

    philo->left_fork = &philo->table->forks[id];
    philo->right_fork = &philo->table->forks[(id + 1) % philo->table->num_philos];

    // Swap forks for even philosophers to avoid deadlock
    if (id % 2 == 0)
    {
...
void init_philosophers(t_table *table)
{
    ...
    while (i < table->num_philos)
    {
        ...
        assign_forks(&table->philos[i]); // Call assign_forks here
